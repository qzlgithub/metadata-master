(function($, window, document) {
    ...
    Paging.prototype = {
        ...
        createDom: function() {
            ...
            totalPages > 1 ? content = '<button class="turnPage" id="prePage">&laquo;</button>' +
                '<div class="pageWrap" style="width:' + wrapLength + 'px">' +
                '<ul id="pageSelect" style="transition:all ' + that.options.slideSpeed + 'ms">' +
                ulDom +
                '</ul></div>' +
                '<button class="turnPage" id="nextPage" style="border-right: solid 1px #ebebeb">&raquo;</button>' +
                jumpDom : content = '';
        ...
        bindEvents: function() {
            var that = this,
                ...
                liWidth = lis.length > 0 ? (lis[0].offsetWidth == 0 ? 60 : lis[0].offsetWidth) : 0, // li的宽度
                totalPages = that.options.totalPages, // 总页数
